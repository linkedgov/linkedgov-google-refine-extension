...
var LinkedGov = {
		...
		checkSchema : function(vocabs, callback) {
			...
			/*
			 * if(rootNode == null){ rootNode = { "nodeType":"cell-as-resource",
			 * "expression":"value", "isRowNumberCell":true, "rdfTypes":[],
			 * "links":[] }; callback(rootNode,true) }else{ callback(rootNode,false) }
			 */
		 ...
		 * findExistingRDF
		 * 
		 * Searches the existing RDF schema for any namespaces relating to the
		 * wizard that's generating RDF.
		 * 
		...
		findExistingRDF : function(schema, namespaces) {
			...
			var self = this;
			/*
			 * Try to find the existing date-time "root node", otherwise create one.
			 */
				...
				for ( var i = 0; i < schema.rootNodes.length; i++) {
					for ( var j = 0; j < schema.rootNodes[i].links.length; j++) {
						for ( var k = 0; k < namespaces.length; k++) {
							if (!foundRootNode
									&& schema.rootNodes[i].links[j].curie
									.split(":")[0] == namespaces[k]) {

								log("Found existing RDF mappings for namespace: "
										+ namespaces[k] + "...");
								foundRootNode = true;
...
LinkedGov.dateTimeWizard = {
		 ...
		 * 
		 * Creates however many of the 6 date time fragments a column possesses -
		 * this is usually used to describe sensible date-times that are not
		 * formattable.
		 */
		...
		/*
		 * Adds a duration string "/P0Y0M0DT0H30M" to the end of a XSD URI.
		 */
		...
		saveRDF : function(rootNode, newRootNode) {
			...
			for ( var i = 0; i < colObjects.length; i++) {
				...
				/*
				 * These RDF fragments are for particular combinations or options
				 * specified that need to be stored carefully.
				 * 
...
LinkedGov.measurementsWizard = {
		...
		saveRDF : function(rootNode, newRootNode) {
			...
			/*
			 * "rootNodes":[ { "nodeType":"cell-as-resource", "expression":"value",
			 * "isRowNumberCell":true, "rdfTypes":[ {
			 * "uri":"http://example.linkedgov.org/EnergyReading",
			 * "curie":"lg:EnergyReading" } ], "links":[ {
			 * "uri":"http://example.linkedgov.org/electricityUsage",
			 * "curie":"lg:electricityUsage", "target":{ "nodeType":"cell-as-blank",
			 * "isRowNumberCell":true, "rdfTypes":[
			 *  ], "links":[ { "uri":"http://rdf.freebase.com/ns/watt-hour",
			 * "curie":"fb:watt-hour", "target":{ "nodeType":"cell-as-literal",
			 * "expression":"value",
			 * "valueType":"http://www.w3.org/2001/XMLSchema#int",
			 * "columnName":"Electricity", "isRowNumberCell":false } } ] } }, {
			 * "uri":"http://example.linkedgov.org/heatUsage",
			 * "curie":"lg:heatUsage", "target":{ "nodeType":"cell-as-blank",
			 * "isRowNumberCell":true, "rdfTypes":[
			 *  ], "links":[ { "uri":"http://rdf.freebase.com/ns/watt-hour",
			 * "curie":"fb:watt-hour", "target":{ "nodeType":"cell-as-literal",
			 * "expression":"value",
			 * "valueType":"http://www.w3.org/2001/XMLSchema#int",
			 * "columnName":"Heat", "isRowNumberCell":false } } ] } }, {
			 * "uri":"http://example.linkedgov.org/gasUsage", "curie":"lg:gasUsage",
			 * "target":{ "nodeType":"cell-as-blank", "isRowNumberCell":true,
			 * "rdfTypes":[
			 *  ], "links":[ { "uri":"http://rdf.freebase.com/ns/watt-hour",
			 * "curie":"fb:watt-hour", "target":{ "nodeType":"cell-as-literal",
			 * "expression":"value",
			 * "valueType":"http://www.w3.org/2001/XMLSchema#int",
			 * "columnName":"Gas", "isRowNumberCell":false } } ] } }, {
			 * "uri":"http://example.linkedgov.org/readingTime",
			 * "curie":"lg:readingTime", "target":{ "nodeType":"cell-as-resource",
			 * "expression":"\"http://reference.data.gov.uk/doc/gregorian-interval/\"+value+\"/P0Y0M0DT0H30M\"",
			 * "columnName":"Date", "isRowNumberCell":false, "rdfTypes":[
			 *  ], "links":[
			 *  ] } } ] } ] }
			 */
			...
			for ( var i = 0; i < colObjects.length; i++) {
				 ...
				 * \""+colObjects[i].name+"\", removing ..."); links.splice(j,1);
				 * j--; }
				 *  }
				 */
...
LinkedGov.addressWizard = {
		...
		initialise : function(elmts) {
			...
			if (self.vars.colObjects.length > 0) {
				 ...
				 * 
				 * Check if a postcode is present in which case a regex pattern
				 * needs to be applied to validate the postcode, otherwise proceed
				 * with storing the RDF.
				 * 
				 * for(var i=0;i<self.vars.colObjects.length;i++){
				 * //log(self.vars.colObjects[i]);
				 * if(self.vars.colObjects[i].containsPostcode ||
				 * self.vars.colObjects[i].part == "postcode"){
				 * self.vars.colObjects[i] =
				 * self.validatePostCode(self.vars.colObjects[i].name); log("here"); } }
				 * 
				 * for(var i=0;i<self.vars.colObjects.length;i++){ log("there");
				 * self.vars.colObjects[i].rdf =
				 * self.makeAddressFragment(self.vars.colObjects[i]); if(i ==
				 * self.vars.colObjects.length-1){ self.saveRDF(); } }
				 */
...
LinkedGov.latLongWizard = {
		...
		saveRDF : function(rootNode, newRootNode) {
			 ...
			 * 
			 *  { "uri":"http://example.linkedgov.org/location",
			 * "curie":"lg:location", "target":{ "nodeType":"cell-as-resource",
			 * "expression":"value+\"#geoPoint\"", "isRowNumberCell":true,
			 * "rdfTypes":[ { "uri":"http://www.w3.org/2003/01/geo/wgs84_pos#Point",
			 * "curie":"geo:Point" } ], "links":[ {
			 * "uri":"http://www.w3.org/2003/01/geo/wgs84_pos#lat",
			 * "curie":"geo:lat", "target":{ "nodeType":"cell-as-literal",
			 * "expression":"value",
			 * "valueType":"http://www.w3.org/2001/XMLSchema#float",
			 * "columnName":"Latitude", "isRowNumberCell":false } }, {
			 * "uri":"http://www.w3.org/2003/01/geo/wgs84_pos#long",
			 * "curie":"geo:long", "target":{ "nodeType":"cell-as-literal",
			 * "expression":"value",
			 * "valueType":"http://www.w3.org/2001/XMLSchema#float",
			 * "columnName":"Longitude", "isRowNumberCell":false } } ] } }
			 * 
			 * 
			 */
...
/*
 * For any columns that do not have RDF stored, save their values in RDF using
 * the column name as the property name.
 */
